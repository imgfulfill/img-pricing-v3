...
import { T, pn, szOrd, fmt, calcLabel, getScenarios, TBL_H } from "../lib/utils";
...
export default function DiscountPage() {
  ...
  const computed = useMemo(() => {
    ...
    return pKeys.map(k => {
      const [prod, sz] = k.split("|||"); const w = productsRawExact(products, prod, sz); if (!w) return null;
      ...
      pricesRawExact(prices, prod, sz).forEach(pr => {
        ...
        let selfLc = 0;
        if (sup.selfShip) { const cs = compRawExact(compPrices, sup.name, prod, sz); selfLc = cs?.shipFirst != null ? cs.shipFirst - (params.a2kDiscount || 0) : 0 }
  ...
  }, [prices, products, supCols, params, labelTiers, compPrices, ckPI, ckSF]);
  ...
  }), [computed, q, fCat, fBrand]);

  const lossCount = data.filter(r => r.allLoss).length;
  const HS = { position: "sticky", zIndex: 3, background: T.sa, textAlign: "right", fontSize: 8, padding: "4px 3px" };
  const presets = [0, 2, 3, 5];
  const CkGroup = ({ label, val, set }) => (
    <div>
      <div style={{ fontSize: 12, color: T.tm, marginBottom: 3 }}>{label}</div>
      <div style={{ display: "flex", gap: 3, alignItems: "center" }}>
        {presets.map(v => <button key={v} className={val === v ? "bp2" : "bg2"} onClick={() => set(v)} style={{ padding: "4px 10px", fontSize: 13 }}>{v}%</button>)}
        <input type="number" value={val} onChange={e => set(pn(e.target.value) || 0)} min={0} max={100} step={1} style={{ width: 50, padding: "4px 6px", fontSize: 13, textAlign: "right" }} />
        <span style={{ fontSize: 12, color: T.td }}>%</span>
      </div>
    </div>
  );
  ...
  return (
    <div className="fade">
      ...
      <div style={{ display: "flex", gap: 6, marginBottom: 10 }}>
        ...
        <select value={fBrand} onChange={e => setFBrand(e.target.value)} style={{ width: 130, fontSize: 13 }}><option value="">All Brand</option>{brands.map(b => <option key={b}>{b}</option>)}</select>
        ...
        <table style={{ fontSize: 12 }}>
          <thead>
            <tr>
              ...
              <th colSpan={2} style={{ position: "sticky", top: 0, zIndex: 3, background: "rgba(59,130,246,.06)", borderLeft: "2px solid " + T.bh, textAlign: "center", fontSize: 10 }}>Giá gốc</th>
              <th colSpan={3} style={{ position: "sticky", top: 0, zIndex: 3, background: "rgba(16,185,129,.08)", borderLeft: "2px solid " + T.ac, textAlign: "center", fontSize: 10, color: T.ac }}>Giá sau CK</th>
              <th colSpan={3} style={{ position: "sticky", top: 0, zIndex: 3, background: "rgba(245,158,11,.06)", borderLeft: "2px solid " + T.w, textAlign: "center", fontSize: 10 }}>LN</th>
            </tr>
            <tr>
              <th style={{ ...HS, top: 31, position: "sticky", left: 0, zIndex: 5, textAlign: "left", minWidth: 150 }}>SP</th>
              <th style={{ ...HS, top: 31, textAlign: "left" }}>Size</th>
              <th style={{ ...HS, top: 31, textAlign: "left", fontSize: 9 }}>Cat</th>
              <th style={{ ...HS, top: 31, borderLeft: "2px solid " + T.bh }}>PrItem</th>
              <th style={{ ...HS, top: 28 }}>Ship1</th>
              <th style={{ ...HS, top: 31, borderLeft: "2px solid " + T.ac }}>PrItem</th>
              <th style={{ ...HS, top: 28 }}>Ship1</th>
              <th style={{ ...HS, top: 31, fontWeight: 700, color: T.ac }}>Tổng</th>
              <th style={{ ...HS, top: 31, borderLeft: "2px solid " + T.w }}>Min</th>
          ...
          <tbody>
            {data.map(r => (
              <tr key={r.k} style={{ background: r.allLoss ? "rgba(239,68,68,.04)" : "" }}>
                <td style={{ position: "sticky", left: 0, zIndex: 1, fontWeight: 500, fontSize: 12, minWidth: 150, whiteSpace: "nowrap",
                  background: r.allLoss ? "rgba(239,68,68,.08)" : T.sf, color: r.allLoss ? T.dg : T.tx }}>{r.prod}</td>
                <td><span className="b bi" style={{ fontSize: 10 }}>{r.sz}</span></td>
                <td style={{ fontSize: 9, color: T.td }}>{r.cat?.replace(/T-shirt all/, "Tee").replace(/Sweat\/Hoodie/, "Hood")}</td>
