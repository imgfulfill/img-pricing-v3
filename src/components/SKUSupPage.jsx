      ...
      {selSup && (() => { const sup = suppliers.find(s => s.name === selSup); const isApi = sup?.api; const skuCount = supStock[selSup]?.rows?.length || 0; return <div style={{ background: T.sf, border: "1px solid " + T.bd, borderRadius: 10, padding: 16 }}>
        ...
        {isApi ? <div style={{ padding: 20, textAlign: "center", color: T.tm }}>
          ...
          {step === "sku" && (() => {
            ...
            const filteredSku = sd.rows.filter((row, i) => {
              if (!skuQ) return true;
              return Object.values(row).some(v => (v || "").toLowerCase().includes(skuQ.toLowerCase()));
            });
            const toggleSkuSel = (i) => { const ns = new Set(skuSel); ns.has(i) ? ns.delete(i) : ns.add(i); setSkuSel(ns) };
            const delSkuRows = async () => {
              if (skuSel.size === 0) return; if (!confirm("Xóa " + skuSel.size + " dòng?")) return;
              const idxSet = new Set([...skuSel].map(si => sd.rows.indexOf(filteredSku[si])));
              const newRows = sd.rows.filter((_, i) => !idxSet.has(i));
              await updateSupStock(selSup, { ...sd, rows: newRows }); setSkuSel(new Set());
            };
            ...
            return <div>
              <div style={{ display: "flex", gap: 6, marginBottom: 8, flexWrap: "wrap", alignItems: "center" }}>
                <input value={skuQ} onChange={e => setSkuQ(e.target.value)} placeholder={"Tìm..."} style={{ flex: 1, minWidth: 150, fontSize: 10 }} />
                <span style={{ fontSize: 10, color: T.tm }}>{filteredSku.length + "/" + sd.rows.length}</span>
                <button className="bg2" onClick={addSkuRow} style={{ fontSize: 9 }}>+ Thêm dòng</button>
                {skuSel.size > 0 && <button className="bdel" onClick={delSkuRows} style={{ fontSize: 9 }}>{"🗑 Xóa " + skuSel.size + " dòng"}</button>}
              </div>
              {sd.rows.length > 0 ? <div style={{ overflow: "auto", maxHeight: "50vh" }}>
                <table style={{ fontSize: 9 }}><thead><tr>
                  <th style={{ width: 25 }}><input type="checkbox" checked={skuSel.size === filteredSku.length && filteredSku.length > 0} onChange={() => { if (skuSel.size === filteredSku.length) setSkuSel(new Set()); else setSkuSel(new Set(filteredSku.map((_, i) => i))) }} /></th>
                  <th style={{ width: 25, fontSize: 7 }}>#</th>
                  {sd.hdr.map((h, i) => <th key={i} style={{ whiteSpace: "nowrap", fontSize: 8 }}>{h}</th>)}
                  <th style={{ width: 50 }}>Act</th>
                </tr></thead><tbody>
                  {filteredSku.map((row, ri) => {
                    const realIdx = sd.rows.indexOf(row);
                    const isEditing = editSkuRow === realIdx;
                    return <tr key={ri} style={{ background: skuSel.has(ri) ? "rgba(59,130,246,.08)" : "" }}>
                      <td><input type="checkbox" checked={skuSel.has(ri)} onChange={() => toggleSkuSel(ri)} /></td>
                      <td style={{ fontSize: 7, color: T.td }}>{realIdx + 1}</td>
                      {sd.hdr.map((h, ci) => <td key={ci}>{isEditing
                        ? <input value={editSkuVals[h] ?? row[h] ?? ""} onChange={e => setEditSkuVals(v => ({ ...v, [h]: e.target.value }))} style={{ width: "100%", fontSize: 8, padding: 1 }} />
                        : <span style={{ fontSize: 8, cursor: "pointer" }} onClick={() => { setEditSkuRow(realIdx); setEditSkuVals({}) }}>{row[h] || ""}</span>
                      }</td>)}
                      <td>{isEditing
                        ? <div style={{ display: "flex", gap: 2 }}>
                            <button className="bp2" style={{ padding: "1px 4px", fontSize: 8 }} onClick={saveEditSku}>✓</button>
                            <button className="bg2" style={{ padding: "1px 4px", fontSize: 8 }} onClick={() => setEditSkuRow(null)}>✕</button>
                          </div>
                        : <button className="bg2" style={{ padding: "1px 4px", fontSize: 8 }} onClick={() => { setEditSkuRow(realIdx); setEditSkuVals({}) }}>✏️</button>
                      }</td>
                    </tr>
                  })}
                </tbody></table>
          ...
          {step === "cfg" && (() => {
            ...
            return <div>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
              <div style={{ fontSize: 12, fontWeight: 600 }}>{"\u2699\uFE0F Column Config"} {curRes ? <span className="b bok">{"Đã cấu hình"}</span> : <span className="b" style={{ background: "rgba(251,191,36,.15)", color: T.w }}>{"Chưa cấu hình"}</span>}</div>
              <div style={{ display: "flex", gap: 4 }}>
                <button className="bg2" style={{ fontSize: 9 }} onClick={autoDetect}>{"\uD83E\uDE84 Tự động đoán"}</button>
                {presetVals && <button className="bp2" style={{ fontSize: 9 }} onClick={async () => {
            ...
            {!supHdr.length && <div style={{ background: "rgba(251,191,36,.08)", border: "1px solid " + T.w, borderRadius: 8, padding: 10, marginBottom: 10, fontSize: 11, color: T.w }}>
              {"\u26A0 Chưa có data SKU " + selSup + ". Nhấn \"Import SKU " + selSup + "\" ở trên để lấy danh sách cột, rồi mới cấu hình được."}
            </div>}
            {!res.productCol && <div style={{ background: "rgba(248,113,113,.08)", border: "1px solid " + T.dg, borderRadius: 8, padding: 10, marginBottom: 10, fontSize: 11, color: T.dg }}>
              ...
              <table style={{ fontSize: 12, width: "100%" }}><tbody>
                {FIELDS.map(([k, label, hint]) =>
                  <tr key={k} style={{ borderBottom: "1px solid " + T.bd }}>
                    <td style={{ fontWeight: 500, padding: "10px 0", minWidth: 160 }}>
                      ...
                      <div style={{ fontSize: 9, color: T.td, fontWeight: 400 }}>{hint}</div>
                    </td>
                ...
                <tr style={{ borderBottom: "1px solid " + T.bd }}>
                  <td style={{ fontWeight: 500, padding: "10px 0" }}>Uppercase
                    <div style={{ fontSize: 9, color: T.td, fontWeight: 400 }}>{"Viết hoa SKU khi xuất"}</div>
                  </td>
                  <td style={{ padding: "10px 0" }}>
          ...
          {step === "stock" && (() => {
            ...
            return <div>
              ...
              <div style={{ display: "flex", gap: 6, marginBottom: 8, flexWrap: "wrap", alignItems: "center" }}>
                <input value={stQ} onChange={e => setStQ(e.target.value)} placeholder={"Tìm..."} style={{ flex: 1, minWidth: 150, fontSize: 10 }} />
                <span style={{ fontSize: 10, color: T.tm }}>{filteredSt.length + "/" + stockD.rows.length}</span>
                <button className="bp2" onClick={() => setShowStImp(!showStImp)} style={{ fontSize: 10 }}>{"📥 Import thêm Stock"}</button>
                <button className="bg2" onClick={addStRow} style={{ fontSize: 9 }}>+ Thêm dòng</button>
                {stSel.size > 0 && <button className="bdel" onClick={delStRows} style={{ fontSize: 9 }}>{"🗑 Xóa " + stSel.size + " dòng"}</button>}
                {stockD.rows.length > 0 && <button className="bdel" onClick={async () => { if (confirm("Xóa TOÀN BỘ stock " + selSup + "?")) { await saveStock({ hdr: [], rows: [] }); setStSel(new Set()) } }} style={{ fontSize: 9 }}>{"Xóa tất cả"}</button>}
              </div>
              {showStImp && <div style={{ marginBottom: 12, background: T.sa, border: "1px solid " + T.bd, borderRadius: 8, padding: 12 }}>
                <div style={{ fontSize: 10, color: T.tm, marginBottom: 6 }}>{"Paste từ Excel. Dòng 1 = header. Import sẽ THÊM vào data hiện tại (không xóa cũ)."}</div>
                <textarea rows={6} style={{ width: "100%", fontSize: 10, fontFamily: "monospace" }} placeholder={"Product\tSize\tColor\n..."} id={"stAppend_" + selSup} />
                <div style={{ marginTop: 6, display: "flex", gap: 6 }}>
                  <button className="bp2" onClick={() => { const ta = document.getElementById("stAppend_" + selSup); if (ta?.value) doAppendImport(ta.value) }} style={{ fontSize: 11 }}>Import thêm</button>
                  <button className="bg2" onClick={() => setShowStImp(false)} style={{ fontSize: 11 }}>{"Hủy"}</button>
                </div>
              </div>}
              {stockD.rows.length > 0 ? <div style={{ overflow: "auto", maxHeight: "50vh" }}>
                <table style={{ fontSize: 9 }}><thead><tr>
                  ...
                  <th style={{ width: 25, fontSize: 7 }}>#</th>
                  {stockD.hdr.map((h, i) => <th key={i} style={{ whiteSpace: "nowrap", fontSize: 8 }}>{h}</th>)}
                  <th style={{ width: 50 }}>Act</th>
                </tr></thead><tbody>
                  {filteredSt.map((row, ri) => {
                    ...
                    return <tr key={ri} style={{ background: stSel.has(ri) ? "rgba(59,130,246,.08)" : "" }}>
                      ...
                      {stockD.hdr.map((h, ci) => <td key={ci}>{isEditing
                        ? <input value={editStVals[h] ?? row[h] ?? ""} onChange={e => setEditStVals(v => ({ ...v, [h]: e.target.value }))} style={{ width: "100%", fontSize: 8, padding: 1 }} />
                        : <span style={{ fontSize: 8, cursor: "pointer" }} onClick={() => { setEditStRow(realIdx); setEditStVals({}) }}>{row[h] || ""}</span>
                      }</td>)}
                      <td>{isEditing
                        ? <div style={{ display: "flex", gap: 2 }}>
                            <button className="bp2" style={{ padding: "1px 4px", fontSize: 8 }} onClick={saveEditSt}>✓</button>
                            <button className="bg2" style={{ padding: "1px 4px", fontSize: 8 }} onClick={() => setEditStRow(null)}>✕</button>
                          </div>
                        : <button className="bg2" style={{ padding: "1px 4px", fontSize: 8 }} onClick={() => { setEditStRow(realIdx); setEditStVals({}) }}>✏️</button>
                      }</td>
                    </tr>
                  })}
                </tbody></table>
