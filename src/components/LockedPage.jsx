...
import { T, szOrd, fmt, TBL_H_TALL } from "../lib/utils";
...
export default function LockedPage() {
  ...
  const GH = { position: "sticky", top: 0, zIndex: 3, background: T.sa, textAlign: "center", fontSize: 12, fontWeight: 600, padding: "6px 4px" };

  return (
    <div className="fade">
      <h2 style={{ fontSize: 22, fontWeight: 700, fontStyle: "italic", marginBottom: 4 }}>Bảng giá IMG</h2>
      <div style={{ fontSize: 14, color: T.tm, marginBottom: 12 }}>{data.length} sản phẩm · Giá chốt snapshot — không thay đổi khi tham số thay đổi</div>
      <div style={{ display: "flex", gap: 6, marginBottom: 10, alignItems: "center" }}>
        <input value={q} onChange={e => setQ(e.target.value)} placeholder="Tìm..." style={{ flex: 1 }} />
        <select value={fCat} onChange={e => setFCat(e.target.value)} style={{ width: 130, fontSize: 13 }}><option value="">All Category</option>{cats.map(c => <option key={c}>{c}</option>)}</select>
        <button className="bp2" onClick={doExport} style={{ fontSize: 13 }}>📥 Export .xlsx</button>
        <button className="bdel" onClick={clearAll} style={{ fontSize: 13 }}>Xóa tất cả</button>
      </div>
      {data.length === 0 ? (
        <div style={{ background: T.sf, border: "1px solid " + T.bd, borderRadius: 10, padding: 40, textAlign: "center", color: T.tm }}>
          <div style={{ fontSize: 30, marginBottom: 8 }}>📋</div>
          <div style={{ fontSize: 15 }}>Chưa có giá chốt. Vào <b>Giá bán đề xuất</b> → nhấn "Paste → Giá chốt"</div>
        </div>
      ) : (
          ...
          <div style={{ overflowX: "auto", overflowY: "auto", maxHeight: TBL_H_TALL }}>
            <table style={{ minWidth: 700, fontSize: 12 }}>
              ...
              <tbody>
                {data.map(r => (
                  <tr key={r.k}>
                    <td style={{ fontSize: 10, color: T.tm, fontWeight: 500 }}>{r.brand}</td>
                    <td style={{ fontWeight: 500, fontSize: 12, whiteSpace: "nowrap" }}>{r.prod}</td>
                    ...
                    <td style={{ textAlign: "center" }}><button className="bdel" style={{ padding: "1px 5px", fontSize: 10 }} onClick={async () => { const [p, s] = r.k.split("|||"); await removeLockedPrice(p, s) }}>🗑</button></td>
                  </tr>
                ))}
              </tbody>
            </table>
