...
import { T, routeOneSKU, mapCell, getScenarios, ltColor, fmt, findPrices, findProduct, calcLabel, matchSupplierProduct, checkStockAvailability } from "../lib/utils";
  ...
  const doPaste = useCallback((txt) => {
    ...
    lines.forEach(l => {
      ...
      activeSups.forEach(sup => {
        const prs = findPrices(prices, sk?.product || "", sk?.size || "")
          .filter(p => sup.id === p.supplierId || sup.name === p.supplierId);
    ...
    Object.entries(orderGroups).forEach(([orderCode, items]) => {
      ...
      const lbl = calcLabel(minWeightOz, labelTiers, params);
      const cheapL = params.cheapPrice || 1.5;
      const expL = lbl ? lbl.expE : params.expSmallest || 3.6;
  ...
  return (
    <div className="fade">
      ...
      {showPaste || orders.length === 0 ? (
        <div>
          <div style={{ fontSize: 12, color: T.tm, marginBottom: 10 }}>{"Paste đơn hàng từ backend IMG (49 cột TSV). Phân theo Order — tất cả item cùng mã đơn đẩy cùng 1 xưởng."}</div>
          <div style={{ background: T.sf, border: "1px solid " + T.bd, borderRadius: 10, padding: 16 }}>
            ...
            <div style={{ display: "flex", gap: 8, marginTop: 8 }}>
              ...
              <div style={{ fontSize: 10, color: T.tm, display: "flex", alignItems: "center" }}>{skuImg.length + " SKU · " + (routeCfg.labelMode || "BLENDED") + " · Phân theo Order"}</div>
            </div>
          </div>
        </div>
      ) : (
        <div>
          <div style={{ display: "flex", gap: 8, marginBottom: 12, flexWrap: "wrap" }}>
            <div style={{ background: T.sf, border: "1px solid " + (fSup === "" ? T.p : T.bd), borderRadius: 8, padding: "8px 14px", cursor: "pointer" }} onClick={() => setFSup("")}>
              <div style={{ fontSize: 18, fontWeight: 700, color: T.ac }}>{orders.length}</div><div style={{ fontSize: 10, color: T.tm }}>{"Tổng item"}</div>
            </div>
            {supKeys.map(s => (
              ...
              <table style={{ fontSize: 10 }}><thead><tr>
                ...
                {filtered.map((o, i) => (
                  <tr key={i} style={{ background: sel.has(i) ? "rgba(59,130,246,.08)" : o.sup === "UNKNOWN" ? "rgba(239,68,68,.06)" : "" }}>
                    ...
                    <td style={{ fontSize: 9, fontFamily: "monospace" }}>{o.orderCode}{o.orderItemCount > 1 && <span style={{ fontSize: 7, color: T.w, marginLeft: 2 }}>{"×" + o.orderItemCount}</span>}</td>
              ...
              return <div key={supName} style={{ background: T.sf, border: "1px solid " + T.bd, borderRadius: 10, padding: 16, marginBottom: 14 }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
                  <div>
                    ...
                    <span style={{ fontSize: 11, color: T.tm, marginLeft: 8 }}>{"(" + items.length + " items)"}</span>
