...
export default function SKUMapPage() {
  ...
  const supIssues = activeSups.map(s => {
    ...
    if (!stock?.rows?.length) return { sup: s.name, why: "chưa import data SKU xưởng" };
    if (!res) return { sup: s.name, why: "chưa cấu hình Column Config" };
    if (!res.productCol) return { sup: s.name, why: "Column Config thiếu Product Column" };
    return null;
  }).filter(Boolean);
  ...
  return (
    <div className="fade">
      ...
      {supIssues.length > 0 && <div style={{ background: "rgba(251,191,36,.08)", border: "1px solid " + T.w, borderRadius: 8, padding: 10, marginBottom: 10, fontSize: 11, color: T.w }}>
        <div style={{ fontWeight: 600, marginBottom: 4 }}>{"\u26A0 Xưởng chưa map được SKU:"}</div>
        {supIssues.map(x => <div key={x.sup} style={{ marginLeft: 8 }}>{"\u2022 " + x.sup + " \u2014 " + x.why}</div>)}
        <div style={{ marginTop: 4, color: T.tm }}>{"\u2192 Vào SKU Xưởng & Template \u2192 chọn xưởng \u2192 tab \u2462 Column Config để khai báo."}</div>
      </div>}
