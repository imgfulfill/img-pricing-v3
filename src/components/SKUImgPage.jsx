...
export default function SKUImgPage() {
  const { skuImg, routeCfg, updateSkuImg, bulkUpsertSkuImg, bulkDeleteSkuImg, clearAllSkuImg, updateRouteCfg } = useData();
    ...
    <div className="fade">
      ...
      {sub === "sku" && <div>
        ...
        {editMode && <div style={{ background: T.sf, border: "1px solid " + T.p, borderRadius: 8, padding: 10, marginBottom: 10, display: "flex", gap: 6, alignItems: "center", flexWrap: "wrap", fontSize: 11 }}>
          ...
          <button className="bdel" onClick={delSelected} style={{ fontSize: 10 }} disabled={!sel.size}>{"🗑 Xóa " + sel.size + " SKU"}</button>
          <button className="bdel" onClick={delAll} style={{ fontSize: 10 }}>{"Xóa toàn bộ (" + skuImg.length + ")"}</button>
        </div>}
