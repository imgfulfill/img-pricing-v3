...
import { T, szOrd, getScenarios, ltColor, fmt, TBL_H } from "../lib/utils";
...
export default function ScenarioPage() {
  ...
  }, [prices, products, suppliers, params, labelTiers, compPrices, side, mode, lockedPrices]);
  ...
  return (
    <div className="fade">
      ...
      <div style={{ fontSize: 14, color: T.tm, marginBottom: 4 }}>{data.length + " SP"}</div>
      <div style={{ fontSize: 12, color: T.w, marginBottom: 10 }}>{"Chế độ Label: " + mode + " · Rank 1 = CP thấp nhất (ưu tiên đẩy đơn)"}</div>
      ...
      <div style={{ display: "flex", gap: 4, marginBottom: 10 }}>
        <button className={side === "1S" ? "bp2" : "bg2"} onClick={() => setSide("1S")} style={{ fontSize: 14, padding: "6px 16px", fontWeight: 600 }}>RANK 1 MẶT</button>
        <button className={side === "2S" ? "bp2" : "bg2"} onClick={() => setSide("2S")} style={{ fontSize: 14, padding: "6px 16px", fontWeight: 600 }}>RANK 2 MẶT</button>
      </div>
      <div style={{ background: T.sf, border: "1px solid " + T.bd, borderRadius: 10, overflow: "hidden" }}>
        <div style={{ overflowX: "auto", overflowY: "auto", maxHeight: TBL_H }}>
          <table style={{ minWidth: maxRank * 320 + 300 }}>
              ...
              <tr>
                ...
                {Array.from({ length: maxRank }, (_, i) => (
                  <th key={i} colSpan={4} style={{ textAlign: "center", position: "sticky", top: 0, zIndex: 3, background: T.sa,
                    borderLeft: "2px solid " + (i === 0 ? T.ac : i === 1 ? T.w : T.bd), color: i === 0 ? T.ac : i === 1 ? T.w : T.tx, fontSize: 15, fontWeight: 700 }}>
                    #{i + 1}
                  </th>
                ))}
              </tr>
              <tr>
                <th style={{ position: "sticky", left: 0, top: 38, zIndex: 5, background: T.sa }} />
                <th style={{ position: "sticky", left: 180, top: 38, zIndex: 5, background: T.sa }} />
                <th style={{ position: "sticky", left: 230, top: 38, zIndex: 5, background: T.sa }} />
                {Array.from({ length: maxRank }, (_, i) => (
                  <React.Fragment key={"h" + i}>
                    <th style={{ position: "sticky", top: 38, zIndex: 3, background: T.sa, borderLeft: "2px solid " + (i === 0 ? T.ac : i === 1 ? T.w : T.bd), fontSize: 10 }}>{"XƯỞNG"}</th>
                    <th style={{ position: "sticky", top: 38, zIndex: 3, background: T.sa, fontSize: 10 }}>LABEL</th>
                    <th style={{ position: "sticky", top: 38, zIndex: 3, background: T.sa, fontSize: 10, textAlign: "right", color: i === 0 ? T.ac : T.tx }}>CP</th>
                    <th style={{ position: "sticky", top: 38, zIndex: 3, background: T.sa, fontSize: 10, textAlign: "right" }}>LN</th>
                  </React.Fragment>
                ))}
              </tr>
            </thead>
            <tbody>
              {data.map(r => (
                <tr key={r.k}>
                  <td style={{ position: "sticky", left: 0, background: T.sf, zIndex: 1, fontWeight: 500, fontSize: 13, whiteSpace: "nowrap" }}>{r.prod}</td>
                  ...
                  {Array.from({ length: maxRank }, (_, i) => {
                    ...
                    return (
                      <React.Fragment key={i}>
                        <td style={{ borderLeft: "2px solid " + (i === 0 ? T.ac : i === 1 ? T.w : T.bd), fontSize: 13 }}>{s.sup}</td>
