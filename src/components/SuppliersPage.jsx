...
export default function SuppliersPage() {
  const { suppliers, prices, products, addSupplier, updateSupplier, deleteSupplier, toggleSupplierActive, addPrice, updatePrice, deletePrice, bulkDeletePrices } = useData();
  ...
  const doBulkDelete = async () => {
    ...
    if (!confirm("Xóa " + bulkSel.size + " dòng giá của " + (selSup?.name || "") + "? Không thể hoàn tác.")) return;
    await bulkDeletePrices([...bulkSel]);
    setBulkSel(new Set());
  };
  ...
  return (
    <div className="fade">
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14 }}>
        ...
        <button className="bp2" onClick={openAddSup} style={{ fontSize: 14 }}>+ Tạo đối tác mới</button>
      </div>

      {/* Add/Edit Supplier Modal */}
      {showAddSup && <div style={{ background: T.sf, border: "1px solid " + T.p, borderRadius: 10, padding: 16, marginBottom: 14 }}>
        <div style={{ fontSize: 16, fontWeight: 600, marginBottom: 10 }}>{editSup ? "Chỉnh sửa đối tác" : "Tạo đối tác mới"}</div>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 10, marginBottom: 10 }}>
          <div>
            <div style={{ fontSize: 12, color: T.tm, marginBottom: 3 }}>Tên đối tác *</div>
            <input value={supForm.name} onChange={e => setSupForm(f => ({ ...f, name: e.target.value }))} placeholder="VD: Lenful" style={{ width: "100%", fontSize: 14 }} />
          </div>
          <div>
            <div style={{ fontSize: 12, color: T.tm, marginBottom: 3 }}>Handling Fee ($)</div>
            <input type="number" step=".01" value={supForm.handling} onChange={e => setSupForm(f => ({ ...f, handling: pn(e.target.value) || 0 }))} style={{ width: "100%", fontSize: 14 }} />
          </div>
          <div>
            <div style={{ fontSize: 12, color: T.tm, marginBottom: 3 }}>Label Ship</div>
            <div style={{ display: "flex", gap: 8, marginTop: 4 }}>
              {[["useCheap", "Cheap"], ["useExp", "Exp"], ["selfShip", "Self"], ["api", "API"]].map(([k, l]) => (
                <label key={k} style={{ fontSize: 13, display: "flex", alignItems: "center", gap: 3, cursor: "pointer" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 10, marginBottom: 10 }}>
          {[["oversizeFee", "Oversize Fee ($)"], ["customFee", "Custom Fee ($)"], ["sleeveFee", "Sleeve Fee ($)"]].map(([k, l]) => (
            <div key={k}>
              <div style={{ fontSize: 12, color: T.tm, marginBottom: 3 }}>{l}</div>
              <input type="number" step=".01" value={supForm[k] ?? ""} placeholder="—" onChange={e => setSupForm(f => ({ ...f, [k]: pn(e.target.value) }))} style={{ width: "100%", fontSize: 14 }} />
            </div>
          ))}
        </div>
        <div style={{ display: "flex", gap: 6 }}>
          <button className="bp2" onClick={saveSup} style={{ fontSize: 14 }}>{editSup ? "Lưu thay đổi" : "Tạo đối tác"}</button>
          <button className="bg2" onClick={() => setShowAddSup(false)} style={{ fontSize: 14 }}>Hủy</button>
        </div>
      </div>}
      ...
      <div style={{ display: "grid", gridTemplateColumns: "260px 1fr", gap: 14 }}>
            ...
            <div key={s.id} onClick={() => { setSel(s.id); setPriceQ(""); setBulkMode(false); setShowAddPrice(false) }}
              ...
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 4 }}>
                ...
                <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
                  <span style={{ fontSize: 12, color: T.p, cursor: "pointer" }} onClick={e => { e.stopPropagation(); openEditSup(s) }} title="Sửa">✏️</span>
        ...
        {!selSup ? (
          <div style={{ background: T.sf, border: "1px solid " + T.bd, borderRadius: 10, padding: 40, textAlign: "center", color: T.tm }}>
            <div style={{ fontSize: 30, marginBottom: 8 }}>👈</div><div style={{ fontSize: 15 }}>Chọn xưởng bên trái</div>
          </div>
        ) : (
          <div style={{ background: T.sf, border: "1px solid " + T.bd, borderRadius: 10, padding: 16 }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
              <div>
                <div style={{ fontSize: 18, fontWeight: 600 }}>{selSup.name} {!selSup.active && <span className="b bdg">Tắt</span>}</div>
                <div style={{ fontSize: 13, color: T.tm }}>{supPrices.length} giá mua · Handling: ${selSup.handling}</div>
              </div>
              <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
                <button className="bg2" onClick={() => openEditSup(selSup)} style={{ fontSize: 12 }}>✏️ Sửa thông tin</button>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 12 }}>
              <div style={{ background: T.sa, borderRadius: 8, padding: 10 }}>
                <div style={{ fontSize: 12, color: T.tm, marginBottom: 4 }}>Label Ship</div>
                <div style={{ display: "flex", gap: 8 }}>
                  ...
                  <label style={{ fontSize: 13, display: "flex", alignItems: "center", gap: 3, cursor: "pointer", color: selSup.api ? T.p : T.td, borderLeft: "1px solid " + T.bd, paddingLeft: 8 }}>
                    <input type="checkbox" checked={!!selSup.api} onChange={e => updateSup(selSup.id, "api", e.target.checked ? 1 : 0)} />🔗 API
                  </label>
                </div>
              </div>
              <div style={{ background: T.sa, borderRadius: 8, padding: 10 }}>
                <div style={{ fontSize: 12, color: T.tm, marginBottom: 4 }}>Phụ phí mặc định</div>
                <div style={{ display: "flex", gap: 6 }}>
                  {[{ k: "oversizeFee", l: "Over" }, { k: "customFee", l: "Cust" }, { k: "sleeveFee", l: "Slv" }].map(f => (
                    <div key={f.k}>
                      <div style={{ fontSize: 9, color: T.td }}>{f.l}</div>
                      <input type="number" step=".01" value={selSup[f.k] ?? ""} placeholder="—"
                        onChange={e => updateSup(selSup.id, f.k, pn(e.target.value))} style={{ width: 70, padding: 3, fontSize: 12 }} />
                    </div>
                  ))}
                </div>
              </div>
            ...
            <div style={{ display: "flex", gap: 6, marginBottom: 10, flexWrap: "wrap" }}>
              <input value={priceQ} onChange={e => setPriceQ(e.target.value)} placeholder="Tìm..." style={{ flex: 1, minWidth: 120, fontSize: 13 }} />
              <select value={pFTech} onChange={e => setPFTech(e.target.value)} style={{ width: 80, fontSize: 12 }}><option value="">Tech</option><option>DTF</option><option>DTG</option></select>
              <select value={pFSize} onChange={e => setPFSize(e.target.value)} style={{ width: 70, fontSize: 12 }}><option value="">Size</option>{priceSizes.map(s => <option key={s}>{s}</option>)}</select>
              <button className={bulkMode ? "bp2" : "bg2"} onClick={() => { setBulkMode(!bulkMode); setBulkSel(new Set()) }} style={{ fontSize: 12 }}>Bulk Edit</button>
              <button className="bp2" onClick={() => { setShowAddPrice(!showAddPrice); setAddQ(""); setAddSel(new Set()) }} style={{ fontSize: 12 }}>{showAddPrice ? "Đóng" : "+ Thêm giá"}</button>
            </div>
            ...
            {showAddPrice && (
              <div style={{ background: T.sa, borderRadius: 8, padding: 12, marginBottom: 10, border: "1px solid " + T.bd }}>
                <div style={{ fontSize: 14, fontWeight: 600, marginBottom: 6 }}>Thêm giá mua — {missingProducts.length} SP chưa có giá</div>
                <div style={{ display: "flex", gap: 6, marginBottom: 8, alignItems: "center" }}>
                  <input value={addQ} onChange={e => setAddQ(e.target.value)} placeholder="Tìm sản phẩm..." style={{ flex: 1, fontSize: 13 }} />
                  <button className="bg2" onClick={selectAllMissing} style={{ fontSize: 10 }}>
                    {addSel.size === filteredMissing.length && filteredMissing.length > 0 ? "Bỏ chọn" : "Chọn tất cả (" + filteredMissing.length + ")"}
                  </button>
                  <button className="bp2" onClick={doAddPrices} disabled={addSel.size === 0} style={{ fontSize: 12, opacity: addSel.size === 0 ? .5 : 1 }}>
                    Thêm {addSel.size} SP
                  </button>
                </div>
                <div style={{ maxHeight: 200, overflowY: "auto", border: "1px solid " + T.bd, borderRadius: 6, background: T.sf }}>
                  {filteredMissing.length === 0 ? (
                    <div style={{ padding: 16, textAlign: "center", color: T.td, fontSize: 13 }}>Tất cả SP đã có giá cho xưởng này</div>
                  ) : filteredMissing.map(p => {
                    const key = p.product + "|||" + p.size;
                    return (
                      <div key={key} onClick={() => toggleAddSel(key)} style={{
                        display: "flex", gap: 8, alignItems: "center", padding: "5px 10px", cursor: "pointer", fontSize: 13,
                        ...
                        <span className="b bi" style={{ fontSize: 10 }}>{p.size}</span>
                        <span style={{ fontSize: 10, color: T.td }}>{p.category}</span>
                      </div>
                    );
                  })}
                </div>
                ...
                {[{ k: "baseCost", l: "Base" }, { k: "handlingFee", l: "Hndl" }, { k: "cost2nd", l: "2nd" }, { k: "cost2ndOver", l: "Over" }, { k: "cost2ndCust", l: "Cust" }, { k: "cost2ndSleeve", l: "Slv" }].map(f => (
                  <div key={f.k}>
                    ...
                    <input type="number" step=".01" value={bulkVals[f.k]} placeholder="—"
                      onChange={e => setBulkVals(v => ({ ...v, [f.k]: e.target.value }))} style={{ width: 70, padding: 3, fontSize: 12 }} />
                  </div>
                ))}
                <button className="bp2" onClick={doBulkApply} style={{ fontSize: 12, padding: "4px 12px" }}>Apply ({bulkSel.size})</button>
                <span style={{ borderLeft: "1px solid " + T.bd, height: 26, margin: "0 2px" }} />
                <button className="bg2" onClick={toggleBulkAll} style={{ fontSize: 12, padding: "4px 10px" }}>
                  {bulkSel.size === supPrices.length && supPrices.length > 0 ? "Bỏ chọn" : "Chọn tất cả (" + supPrices.length + ")"}
                </button>
                <button className="bdel" onClick={doBulkDelete} style={{ fontSize: 12, padding: "4px 12px" }} disabled={!bulkSel.size}>
                  {"🗑 Xóa " + bulkSel.size + " giá"}
                </button>
              </div>
            )}
              ...
              <table><thead><tr>
                ...
                {supPrices.map(p => editPriceId === p.id ? (
                  <tr key={p.id} style={{ background: "rgba(59,130,246,.08)" }}>
                    {bulkMode && <td />}
                    <td><input value={editPriceRow.product || ""} onChange={e => setEditPriceRow(r => ({ ...r, product: e.target.value }))} style={{ width: "100%", fontSize: 12, padding: 2 }} /></td>
                    <td><input value={editPriceRow.size || ""} onChange={e => setEditPriceRow(r => ({ ...r, size: e.target.value }))} style={{ width: 50, fontSize: 12, padding: 2 }} /></td>
                    {["baseCost", "handlingFee", "totalCost", "cost2nd", "cost2ndOver", "cost2ndCust", "cost2ndSleeve"].map(k => (
                      <td key={k}><input type="number" step=".01" value={editPriceRow[k] != null ? editPriceRow[k] : ""} placeholder="—"
                        ...
                        style={{ width: 60, fontSize: 12, padding: 2, textAlign: "right", background: k === "totalCost" ? T.bd : undefined }} /></td>
                    ))}
                    <td><div style={{ display: "flex", gap: 2 }}>
                      <button className="bp2" style={{ padding: "2px 6px", fontSize: 10 }} onClick={e => { e.stopPropagation(); saveEditPrice() }}>✓</button>
                      <button className="bg2" style={{ padding: "2px 6px", fontSize: 10 }} onClick={() => setEditPriceId(null)}>✕</button>
                    </div></td>
                  </tr>
                ) : (
                  <tr key={p.id}>
                    ...
                    <td style={{ maxWidth: 180, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", fontSize: 12 }}>{p.product}</td>
                    ...
                    <td><div style={{ display: "flex", gap: 2 }}>
                      <button className="bg2" style={{ padding: "1px 5px", fontSize: 10 }} onClick={() => startEditPrice(p)}>✏️</button>
                      <button className="bdel" style={{ padding: "1px 5px", fontSize: 10 }} onClick={async () => { if (confirm("Xóa giá này?")) await deletePrice(p.id) }}>🗑</button>
                    </div></td>
                  </tr>
                ))}
              </tbody></table>
