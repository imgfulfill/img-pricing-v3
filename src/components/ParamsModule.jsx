...
export default function ParamsModule() {
    ...
    return (
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "10px 0", borderBottom: "1px solid " + T.bd }}>
        <div style={{ flex: 1 }}>
          ...
          {desc && <div style={{ fontSize: 12, color: T.td, marginTop: 1 }}>{desc}</div>}
        </div>
        <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
          ...
          <span style={{ fontSize: 12, color: T.td, minWidth: 40 }}>{unit}</span>
        </div>
      </div>
    );
  };
  ...
  return (
    <div className="fade">
      ...
      <div style={{ fontSize: 14, color: T.tm, marginBottom: 14 }}>Thay đổi → ảnh hưởng tính toán Label, Kịch bản SX, Giá đề xuất.</div>
      <div style={{ background: T.sf, border: "1px solid " + T.bd, borderRadius: 10, padding: 16, marginBottom: 16 }}>
        <div style={{ fontSize: 15, fontWeight: 600, marginBottom: 8 }}>🚚 Chế độ Label</div>
        <div style={{ display: "flex", gap: 6 }}>{["BLENDED", "EXP_ONLY", "CHEAP_ONLY"].map(m => <button key={m} className={labelMode === m ? "bp2" : "bg2"} onClick={() => setLM(m)} style={{ fontSize: 14, padding: "8px 16px" }}>{m}</button>)}</div>
      </div>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
        <div style={{ background: T.sf, border: "1px solid " + T.bd, borderRadius: 10, padding: 16 }}>
          <div style={{ fontSize: 15, fontWeight: 600, color: T.p, marginBottom: 8 }}>Label & Mix</div>
        ...
        <div style={{ background: T.sf, border: "1px solid " + T.bd, borderRadius: 10, padding: 16 }}>
          <div style={{ fontSize: 15, fontWeight: 600, color: T.p, marginBottom: 8 }}>Pricing & Chiến lược</div>
      ...
      <div style={{ background: T.sf, border: "1px solid " + T.bd, borderRadius: 10, padding: 16, marginTop: 16 }}>
        <div style={{ fontSize: 15, fontWeight: 600, color: T.p, marginBottom: 8 }}>Ship First / Ship Add per Category</div>
        <div style={{ maxHeight: 300, overflowY: "auto" }}>
          <table><thead><tr>
            ...
            {Object.entries(params.categoryShip || {}).map(([cat, v]) => (
              <tr key={cat}>
                <td style={{ fontWeight: 500, fontSize: 14 }}>{cat}</td>
                <td style={{ textAlign: "right" }}>
                  <input type="number" step=".01" value={v.s1} style={{ width: 70, textAlign: "right", padding: 3, fontSize: 13 }}
                    onChange={e => { const cs = { ...params.categoryShip }; cs[cat] = { ...cs[cat], s1: pn(e.target.value) || 0 }; u("categoryShip", cs) }} />
                </td>
                <td style={{ textAlign: "right" }}>
                  <input type="number" step=".01" value={v.sa} style={{ width: 70, textAlign: "right", padding: 3, fontSize: 13 }}
