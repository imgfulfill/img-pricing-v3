...
import { T, fmt, szOrd, calcLabel, ltColor, TBL_H_TALL } from "../lib/utils";
import { productsRawExact, pricesRawExact, compRawExact } from "../lib/indexes";
...
export default function ComparePage() {
  ...
  /* ── TẦNG 1 — phần tính NẶNG. Không còn phụ thuộc ô tìm kiếm,
     nên gõ vào ô lọc không làm tính lại toàn bộ bảng nữa. */
  const computed = useMemo(() => {
    ...
    return pKeys.map(k => {
      ...
      pricesRawExact(prices, prod, sz).forEach(pr => {
        ...
        sup.cols.forEach(lt => {
          let lc = 0;
          if (lt === "Self") { const cs = compRawExact(compPrices, sup.name, prod, sz); lc = cs?.shipFirst != null ? cs.shipFirst - (params.a2kDiscount || 0) : 0 }
  ...
  }, [prices, products, supCols, params, labelTiers, compPrices]);

  /* ── TẦNG 2 — chỉ LỌC. Rất rẻ, chạy mỗi lần gõ. */
  ...
  return (
    <div className="fade">
      ...
      <div style={{ fontSize: 14, color: T.tm, marginBottom: 10 }}>{data.length} SP</div>
      <div style={{ display: "flex", gap: 6, marginBottom: 10, flexWrap: "wrap", alignItems: "center" }}>
        <input value={q} onChange={e => setQ(e.target.value)} placeholder="Tìm..." style={{ flex: 1, minWidth: 150 }} />
        <select value={fType} onChange={e => setFType(e.target.value)} style={{ width: 100, fontSize: 13 }}><option value="">DTF/DTG</option><option>DTF</option><option>DTG</option></select>
        <select value={fCat} onChange={e => setFCat(e.target.value)} style={{ width: 120, fontSize: 13 }}><option value="">All</option>{cats.map(c => <option key={c}>{c}</option>)}</select>
      </div>
          ...
          <table style={{ minWidth: totalSupCols * 160 + 390 }}>
            <thead>
              <tr>
                ...
                <th colSpan={totalSupCols} style={{ textAlign: "center", position: "sticky", top: 0, zIndex: 3, background: T.sa, fontSize: 14, fontWeight: 600, color: T.tx, letterSpacing: 1, padding: "8px 12px", borderLeft: "2px solid " + T.bh }}>
                  CP TỔNG — IN 1 MẶT
                </th>
                <th colSpan={totalSupCols} style={{ textAlign: "center", position: "sticky", top: 0, zIndex: 3, background: T.sa, fontSize: 14, fontWeight: 600, color: T.w, letterSpacing: 1, padding: "8px 12px", borderLeft: "3px solid " + T.w }}>
                  CP TỔNG — IN 2 MẶT
                </th>
              </tr>
              <tr>
                <th style={{ position: "sticky", left: SL[0], top: 39, zIndex: 5, background: T.sa, minWidth: 180 }} rowSpan={2}>SP</th>
                <th style={{ position: "sticky", left: SL[1], top: 39, zIndex: 5, background: T.sa, minWidth: 55, textAlign: "center" }} rowSpan={2}>SIZE</th>
                <th style={{ position: "sticky", left: SL[2], top: 39, zIndex: 5, background: T.sa, textAlign: "center", minWidth: 45 }} rowSpan={2}>OZ</th>
                <th style={{ position: "sticky", left: SL[3], top: 39, zIndex: 5, background: T.sa, textAlign: "center", minWidth: 55 }} rowSpan={2} title="Giá label Exp thực tế (sau reweigh)">EXPHQ</th>
                <th style={{ position: "sticky", left: SL[4], top: 39, zIndex: 5, background: T.sa, textAlign: "center", minWidth: 50 }} rowSpan={2} title="Giá label Blended">BLD</th>
                {supCols.map(s => <th key={"1s_" + s.name} colSpan={s.cols.length} style={{ textAlign: "center", borderLeft: "2px solid " + T.bh, position: "sticky", top: 39, zIndex: 3, background: T.sa, fontSize: 13, fontWeight: 600, color: T.tx }}>{s.name.toUpperCase()}</th>)}
                {supCols.map(s => <th key={"2s_" + s.name} colSpan={s.cols.length} style={{ textAlign: "center", borderLeft: s === supCols[0] ? "3px solid " + T.w : "2px solid " + T.bh, position: "sticky", top: 39, zIndex: 3, background: T.sa, fontSize: 13, fontWeight: 600, color: T.w }}>{s.name.toUpperCase()}</th>)}
              </tr>
              <tr>
                {supCols.map(s => s.cols.map((lt, i) => <th key={"1s_" + s.name + "_" + lt} style={{ textAlign: "center", fontSize: 10, position: "sticky", top: 72, zIndex: 3, background: T.sa, color: ltColor(lt), fontWeight: 600, borderLeft: i === 0 ? "2px solid " + T.bh : undefined }}>{lt.toUpperCase()}</th>))}
                {supCols.map((s, si) => s.cols.map((lt, i) => <th key={"2s_" + s.name + "_" + lt} style={{ textAlign: "center", fontSize: 10, position: "sticky", top: 72, zIndex: 3, background: T.sa, color: ltColor(lt), fontWeight: 600, borderLeft: (si === 0 && i === 0) ? "3px solid " + T.w : i === 0 ? "2px solid " + T.bh : undefined }}>{lt.toUpperCase()}</th>))}
              </tr>
            </thead>
            <tbody>
              {data.map(r => (
                <tr key={r.k}>
                  <td style={{ position: "sticky", left: SL[0], background: T.sf, zIndex: 1, fontWeight: 500, fontSize: 13, whiteSpace: "nowrap", verticalAlign: "middle" }}>{r.prod}</td>
      ...
      <div style={{ fontSize: 12, color: T.tm, marginTop: 6 }}><span style={{ color: T.ac, fontWeight: 600 }}>■</span> Chi phí thấp nhất · Phần trái: IN 1 MẶT · Phần phải (<span style={{ color: T.w }}>vàng</span>): IN 2 MẶT</div>
    </div>
  );
}
