...
export function matchSupplierProduct(imgProduct, supProductCol, supRow) {
  const imgFull = normP(imgProduct);
  const supProd = normP(supRow[supProductCol] || "");
...
export function checkStockAvailability(product, size, color, supplierName, stockData) {
  ...
  const prodTech = prodLow.includes("dtg") ? "dtg" : prodLow.includes("dtf") ? "dtf" : prodLow.includes("sublimation") ? "sublimation" : "";
  ...
  const productRows = sd.rows.filter(row => {
    const rowProd = normP(row[prodCol] || Object.values(row)[0] || "");
    if (!rowProd) return false;
    // Normalized exact or contains match
    ...
    const rowTech = rowProd.includes("dtg") ? "dtg" : rowProd.includes("dtf") ? "dtf" : rowProd.includes("sublimation") ? "sublimation" : "";
    if (prodNums.length > 0 && rowNums.length > 0 && prodNums.some(n => rowNums.includes(n)) && prodTech === rowTech) return true;
    // For products without model numbers (like SOCCER JERSEY), use normalized name matching
    if (prodNums.length === 0 && rowNums.length === 0) {
      // Compare key words
      const prodWords = prodLow.replace(/[^a-z0-9 ]/g, " ").split(/\s+/).filter(w => w.length > 2);
      const rowWords = rowProd.replace(/[^a-z0-9 ]/g, " ").split(/\s+/).filter(w => w.length > 2);
      const matchCount = prodWords.filter(w => rowWords.includes(w)).length;
      if (matchCount >= 2 && matchCount >= Math.min(prodWords.length, rowWords.length) * 0.6) return true;
    }
    return false;
  });
